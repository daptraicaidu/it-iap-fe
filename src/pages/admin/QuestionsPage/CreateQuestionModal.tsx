...
const CreateQuestionModal = ({ isOpen, onClose, onSuccess }: CreateQuestionModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setToast({ type: "success", message: t("createModal.success") });
      setTimeout(() => {
        resetForm();
        onClose();
        onSuccess();
      ...
      if (axios.isAxiosError(err) && err.response?.data) {
        ...
        if (resData.data && typeof resData.data === "object") {
          ...
          setToast({ type: "error", message: t("createModal.error") });
        }
      } else {
        setToast({ type: "error", message: t("createModal.error") });
      }
        ...
        <div className="max-h-[70vh] overflow-y-auto px-6 py-5">
          <div className="space-y-4">
            ...
            <div>
              ...
              />
              {errors.suggestedAnswer && (
                <p className="mt-1 text-xs text-rose-600">{errors.suggestedAnswer}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errors.hintContent && (
                <p className="mt-1 text-xs text-rose-600">{errors.hintContent}</p>
              )}
            </div>

            {/* Position + Level + Category (3-col grid) */}
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              ...
              <div>
                ...
                <select
                  ...
                  className={`w-full appearance-none rounded-lg border bg-white px-3.5 py-2 text-sm text-zinc-900 outline-none transition-colors focus:ring-1 ${
                    errors.position
                      ? "border-rose-300 focus:border-rose-400 focus:ring-rose-400"
                      : "border-zinc-200 focus:border-zinc-400 focus:ring-zinc-400"
                  }`}
            ...
            <div>
              ...
              <input
                ...
                className={`w-full rounded-lg border bg-white px-3.5 py-2 text-sm text-zinc-900 tabular-nums outline-none transition-colors placeholder:text-zinc-400 focus:ring-1 ${
                  errors.timeLimit
                    ? "border-rose-300 focus:border-rose-400 focus:ring-rose-400"
                    : "border-zinc-200 focus:border-zinc-400 focus:ring-zinc-400"
                }`}
              />
              {errors.timeLimit && (
                <p className="mt-1 text-xs text-rose-600">{errors.timeLimit}</p>
              )}
            </div>
          </div>
        </div>
